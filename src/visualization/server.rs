use super::dump::{dump_voter_timestamp, dump_account, dump_ledger};
        ...
        thread::spawn(move || {
            for req in server.handle.incoming_requests() {
                ...
                thread::spawn(move || {
                    ...
                    let account_addr: Option<Address> = params
                        .get("address")
                    ...
                    let key: Option<H256> = params
                        .get("key")
                        .and_then(|s| hex::decode(s).ok()
                            .and_then(|v| if v.len() == H256::len_bytes() {
                                Some(H256::from_slice(v.as_slice()))
                            } else {
                                None
                            }));
                    match url.path() {
                        ...
                        "/ledger.json" => serve_dynamic_file!(
                            req,
                            dump_ledger(&blockchain, &blockdb, limit),
                            "application/json",
                            addr
                        ),
                        ...
                        "/account.json" => {
                            ...
                            req,
                            match &account_addr {
                                Some(account_addr) => dump_account(&statedb, account_addr, &key),
                                _ => "Account Dump error".to_string(),
                            },
                            "application/json",
                            addr
                        )},
                        ...
                        //                    }
                        "/account" => {
                            serve_dynamic_file!(req, include_str!("account.html"), "text/html", addr)
                        }
                        "/" => {
