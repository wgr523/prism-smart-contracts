...
# change the template to yours. it should be Amazon EC2 c5d.4xlarge
LAUNCH_TEMPLATE=lt-02ad9e8b2cee5a645 #4xlarge
...
function prepare_keypair
{
	# $1: keypairs per node
	if [ $# -ne 1 ]; then
		tput setaf 1
		echo "Required: number of keypairs per node"
		tput sgr0
		exit 1
	fi
	echo "Deleting existing keypairs and generating new ones"
	rm -rf keypairs
    python3 scripts/gen_prism_keypairs.py $1
	tar cvzf keypair.tar.gz -C keypairs . &> /dev/null
}

function prepare_payload
{
	# $1: topology file to use, $2: keypairs per node
	if [ $# -eq 0 ]; then
		tput setaf 1
		echo "Required: topology file, Optinal: number of keypairs per node (default 1)"
		tput sgr0
		exit 1
	fi
    ...
    cp keypair.tar.gz payload/keypair.tar.gz
	...
	python3 scripts/gen_prism_payload.py instances.txt $1 $2
	...
	do
		...
		tar cvf payload/$id.tar.gz -C payload/$id . &> /dev/null
		rm -rf payload/$id
	done
...
function prepare_small_payload
{
	# $1: topology file to use, $2: keypairs per node
	if [ $# -eq 0 ]; then
		tput setaf 1
...
function sync_payload
{
	...
	aws s3 rm --quiet --recursive s3://prism-smart-contract/payload
	aws s3 sync --quiet payload s3://prism-smart-contract/payload
	echo "Downloading payload on each instance"
	execute_on_all get_payload
}

function sync_small_payload
{
	echo "Uploading payload to S3"
	aws s3 rm --quiet --recursive s3://prism-smart-contract/payload --exclude "*common*"
	aws s3 sync --quiet payload s3://prism-smart-contract/payload --exclude "*common*"
	echo "Downloading payload on each instance"
	execute_on_all get_payload_small
}

function get_payload_single
{
	ssh $1 -- "rm -f /home/ubuntu/*.tar.gz && rm -rf /home/ubuntu/payload && wget https://prism-smart-contract.s3.amazonaws.com/payload/$1.tar.gz -O local.tar.gz && wget https://prism-smart-contract.s3.amazonaws.com/payload/common.tar.gz && wget https://prism-smart-contract.s3.amazonaws.com/payload/keypair.tar.gz && mkdir -p /home/ubuntu/payload && tar xf local.tar.gz -C /home/ubuntu/payload && tar xf common.tar.gz -C /home/ubuntu/payload && tar xf keypair.tar.gz -C /home/ubuntu/payload"
}

function get_payload_small_single
{
	ssh $1 -- "rm -f /home/ubuntu/local.tar.gz && rm -rf /home/ubuntu/payload/prism-payload && wget https://prism-smart-contract.s3.amazonaws.com/payload/$1.tar.gz -O local.tar.gz && mkdir -p /home/ubuntu/payload && tar xf local.tar.gz -C /home/ubuntu/payload"
}

function install_perf_single
{
	ssh $1 -- 'rm -f rustfilt && rm -rf inferno && sudo apt-get update -y && sudo apt-get install linux-tools-aws linux-tools-4.15.0-1051-aws binutils -y && wget https://github.com/yangl1996/rustfilt/releases/download/1/rustfilt && wget https://github.com/yangl1996/inferno/releases/download/bin/linux64.tar.gz && mkdir -p inferno && tar xf linux64.tar.gz -C inferno && chmod +x rustfilt && chmod +x inferno/* && sudo apt-get install -y c++filt && echo export PATH=$PATH:/home/ubuntu:/home/ubuntu/inferno >> /home/ubuntu/.profile'
}
...
{
	ssh $1 -- 'diskname=$(lsblk | grep 372.5G | cut -f 1 -d " ") && sudo rm -rf /tmp/prism && sudo mkdir -m 777 /tmp/prism && sudo mkfs -F -t ext4 /dev/$diskname && sudo mount /dev/$diskname /tmp/prism && sudo chmod 777 /tmp/prism'
}
...
function enable_tcp_bbr_single
{
	ssh $1 -- "sudo sysctl net.core.default_qdisc=fq && sudo sysctl net.ipv4.tcp_congestion_control=bbr"
}

function increase_nofile_single
{
	ssh $1 -- "sudo bash -c \"echo 'ubuntu soft nofile 640000' >> /etc/security/limits.conf && echo 'ubuntu hard nofile 640000' >> /etc/security/limits.conf\""
}
...
function start_transactions_single
{
	...
	curl -s "http://$3:$4/transaction-generator/set-value-distribution?min=1&max=1&distribution=uniform"
	curl -s "http://$3:$4/transaction-generator/start?throttle=10000"
}

function start_mining_single
{
	#curl -s "http://$3:$4/miner/start?lambda=390000&lazy=true&prob=0.9"
	curl -s "http://$3:$4/miner/start?lambda=390000&lazy=false&prob=0.0"
}
...
function set_tx_type
{
	...
	for node in $nodes; do
		...
		local apiport
		IFS=',' read -r name host pubip _ _ apiport _ <<< "$node"
		curl -s "http://$pubip:$apiport/transaction-generator/set-transaction-type?type=$txtype" &> /dev/null &
		pids="$pids $!"
	done
	for pid in $pids; do
		wait $pid
...
case "$1" in
	help)
		...
		Run Experiment
		  ...
		  run-exp-contract           Run the experiment with a certain type of smart contract transaction
    ...
    enable-bbr)
        execute_on_all enable_tcp_bbr ;;
	cores-on)
		execute_on_all enable_cpu_cores $2 ;;
    increase-nofile)
        execute_on_all increase_nofile ;;
	get-perf)
		show_performance $2 ;;
	show-vis)
