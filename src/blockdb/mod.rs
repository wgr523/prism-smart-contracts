...
use rocksdb::{self, ColumnFamilyDescriptor, Options, SliceTransform, DB, IteratorMode};
...
impl BlockDatabase {
    ...
    pub fn load<P: AsRef<std::path::Path>>(
        ...
        let block_arrival_order_cf = db.db.cf_handle(BLOCK_ARRIVAL_ORDER_CF).unwrap();
        let iter = db.db.iterator_cf(block_arrival_order_cf, IteratorMode::End).unwrap();
        let counter = iter.count() as u64;
        db.count.store(counter, Ordering::Relaxed);
        Ok(db)
    }
