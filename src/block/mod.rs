...
pub mod tests {
    ...
    #[test]
    fn ser() {
        let voter = voter_block(Default::default(), Default::default(), Default::default(), Default::default(), vec![Default::default(), Default::default()]);
        println!("Block: {:?}", voter);
        println!("Block content: {:?}", voter.content);
        println!("Content Size: {}", bincode::serialize(&voter.content).unwrap().len());
        let h: H256 = Default::default();
        println!("Hash Deser Size: {}", bincode::serialize(&h).unwrap().len());
        println!("Hash Deser: {:?}", bincode::serialize(&h).unwrap());
    }
}
